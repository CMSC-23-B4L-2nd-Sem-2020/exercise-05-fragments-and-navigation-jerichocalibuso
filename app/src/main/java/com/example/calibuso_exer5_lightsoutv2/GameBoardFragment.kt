...
class GameBoardFragment : Fragment() {
  ...
  fun getTaps(tapsCount: Int): String{
    return tapsCount.toString()
  }
  ...
  private fun play(view: ImageView) {
    ...
    if (onCount == 0) {  // if all buttons are turned off, game is over
      ...
      //ADD taps count to congratsScreen
      val bundle: Bundle = Bundle()
      bundle.putString("count", tapsCount.toString())
      view.findNavController()
        .navigate(R.id.action_gameBoardFragment2_to_congtrasScreenFragment, bundle)
    }
  ...
  private fun getButton(
    row: Int,
    column: Int
  ): ImageView {  //returns button id only using the row and column
    val listOfButtons: List<List<ImageView>> = listOf(
      listOf(
        binding.untapped11Image,
